...
* **dns_lab**: This is a lab composed by 3 web servers, an "attacking" *Kali* and a *Bind9* DNS container attached to every other container. This way, using *web1.ssi.es* access the first container, while *web2.ssi.es* and *web3.ssi.es* accesses the second and the third. *kali.ssi.es* can be used to address the *Kali* container. This setup makes very easy to create labs that can use DNS names instead of IPs, as you have only to change the contents of the *dns_conf* lab folder to adapt them to your needs.
